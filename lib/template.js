...
export function renderProposal(proposal, { metaPixelId = '' } = {}) {
  ...
  const note = proposal.personalNote || '';
...
.cover-note{background:rgba(141,141,255,.08);border-left:3px solid #8D8DFF;border-radius:0 6px 6px 0;padding:10px 14px;margin-bottom:16px;font-size:9pt;color:#c5d3f8;line-height:1.6;font-style:italic}
/* ── Section pages ── */
...
<div class="page cover" style="padding:0;min-height:267mm">
  <div class="cover-left">
    ...
    <div>
      ...
      ${note ? `<div class="cover-note">${note.replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/\n/g,'<br>')}</div>` : ''}
